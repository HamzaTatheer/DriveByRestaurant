    ...
    describe("Given that Database has been created with initial seed",()=>{
        ...
        describe("Correct User details provided for both sign up and login",()=>{
            ...
            it("Correct Sign Up Info Provided",async () =>{
                    let request = supertest(app);
                    await request.post("/api/customer/signup").attach('avatar',"./api_tests/images/customer_pfp.jpg").field("name","omahamza").field("phone",phone).field("password",password).expect(200).then((res)=>{
